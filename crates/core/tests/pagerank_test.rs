...
fn pagerank_simple_chain() {
    ...
    let result = db
        .query("CALL pagerank('E') YIELD node_id, score")
        .unwrap();
...
fn pagerank_isolated_nodes() {
    ...
    assert!(
        (s1 - s2).abs() < 0.01,
        "isolated nodes should have equal score: {} vs {}",
        s1,
        s2
    );
}
...
fn pagerank_scores_sum_to_one() {
    ...
    db.execute("MATCH (a:N), (b:N) WHERE a.id = 2 AND b.id = 3 CREATE (a)-[:E]->(b)")
        .unwrap();
    db.execute("MATCH (a:N), (b:N) WHERE a.id = 3 AND b.id = 1 CREATE (a)-[:E]->(b)")
        .unwrap();
...
fn pagerank_cycle_graph() {
    ...
    for s in &scores {
        assert!(
            (s - 1.0 / 3.0).abs() < 0.01,
            "cycle node should have score ~1/3, got {}",
            s
        );
    }
}
