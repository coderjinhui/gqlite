...
fn setup_city_graph() -> Database {
    ...
    db.execute("CREATE NODE TABLE City(id INT64, name STRING, PRIMARY KEY(id))")
        .unwrap();
    db.execute("CREATE REL TABLE ROAD(FROM City TO City, distance DOUBLE)")
        .unwrap();
...
fn dijkstra_no_path() {
    ...
    db.execute("CREATE NODE TABLE N(id INT64, PRIMARY KEY(id))")
        .unwrap();
    db.execute("CREATE REL TABLE E(FROM N TO N, w DOUBLE)")
        .unwrap();
    db.execute("CREATE (n:N {id: 1})").unwrap();
    db.execute("CREATE (n:N {id: 2})").unwrap();
    // No edges — expect 0 rows
    let result = db
        .query("CALL dijkstra(1, 2, 'E', 'w') YIELD path, cost")
        .unwrap();
...
fn dijkstra_direct_is_shortest() {
    ...
    db.execute("CREATE (n:N {id: 2})").unwrap();
    db.execute(
        "MATCH (a:N), (b:N) WHERE a.id = 1 AND b.id = 2 CREATE (a)-[:E {w: 5.0}]->(b)",
    )
    .unwrap();
    let result = db
        .query("CALL dijkstra(1, 2, 'E', 'w') YIELD cost")
        .unwrap();
    assert_eq!(result.num_rows(), 1);
...
fn dijkstra_same_source_and_target() {
    ...
    let result = db
        .query("CALL dijkstra(1, 1, 'E', 'w') YIELD path, cost")
        .unwrap();
    assert_eq!(result.num_rows(), 1);
