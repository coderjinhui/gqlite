...
fn subquery_basic() {
    let db = setup();
    let result = db.query("CALL { MATCH (n:N) RETURN n.name AS name } RETURN name").unwrap();
    assert_eq!(result.num_rows(), 3);
    let mut names: Vec<String> =
        result.rows().iter().map(|r| r.get_string(0).unwrap().to_string()).collect();
    names.sort();
...
fn subquery_with_limit() {
    let db = setup();
    let result = db
        .query("CALL { MATCH (n:N) RETURN n.name AS name ORDER BY n.name LIMIT 1 } RETURN name")
        .unwrap();
...
fn subquery_with_where_after() {
    let db = setup();
    let result = db
        .query("CALL { MATCH (n:N) RETURN n.id AS id, n.name AS name } WHERE id > 1 RETURN name")
        .unwrap();
...
fn subquery_empty_result() {
    let db = setup();
    let result = db
        .query("CALL { MATCH (n:N) WHERE n.id > 999 RETURN n.name AS name } RETURN name")
        .unwrap();
...
fn subquery_with_preceding_match() {
    ...
    let result = db
        .query("MATCH (a:N) CALL { MATCH (b:N) RETURN count(b) AS total } RETURN a.name, total")
        .unwrap();
