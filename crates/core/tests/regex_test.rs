...
fn regex_basic_match() {
    ...
    let result =
        db.query("MATCH (n:N) WHERE n.name =~ 'A.*' RETURN n.name ORDER BY n.name").unwrap();
...
fn regex_case_insensitive() {
    ...
    db.execute("CREATE NODE TABLE N(id INT64, name STRING, PRIMARY KEY(id))").unwrap();
    db.execute("CREATE (n:N {id: 1, name: 'Alice'})").unwrap();
    let result = db.query("MATCH (n:N) WHERE n.name =~ '(?i)alice' RETURN n.name").unwrap();
...
fn regex_no_match() {
    ...
    db.execute("CREATE NODE TABLE N(id INT64, name STRING, PRIMARY KEY(id))").unwrap();
    db.execute("CREATE (n:N {id: 1, name: 'Alice'})").unwrap();
    let result = db.query("MATCH (n:N) WHERE n.name =~ '^B.*' RETURN n.name").unwrap();
