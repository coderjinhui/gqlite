...
fn in_basic_int_list() {
    ...
    let result = db
        .query("MATCH (n:N) WHERE n.id IN [2, 4] RETURN n.id ORDER BY n.id")
        .unwrap();
...
fn not_in_list() {
    ...
    let result = db
        .query("MATCH (n:N) WHERE n.id NOT IN [2, 4] RETURN n.id ORDER BY n.id")
        .unwrap();
...
fn in_empty_list() {
    ...
    let result = db
        .query("MATCH (n:N) WHERE n.id IN [] RETURN n.id")
        .unwrap();
