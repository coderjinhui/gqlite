...
fn case_searched_basic() {
    ...
    db.execute("CREATE NODE TABLE Person(id INT64, age INT64, PRIMARY KEY(id))").unwrap();
...
fn case_simple_form() {
    let db = Database::in_memory();
    db.execute("CREATE NODE TABLE Status(id INT64, code INT64, PRIMARY KEY(id))").unwrap();
...
fn case_no_else_returns_null() {
    ...
    let result =
        db.query("MATCH (i:Item) RETURN CASE WHEN i.val > 100 THEN 'big' END AS label").unwrap();
