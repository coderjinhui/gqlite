...
fn degree_centrality_basic() {
    ...
    let result = db
        .query("CALL degree_centrality('E') YIELD node_id, out_degree, in_degree")
        .unwrap();
...
fn degree_centrality_yield_subset() {
    ...
    db.execute("CREATE REL TABLE E(FROM N TO N)").unwrap();
    db.execute("CREATE (n:N {id: 1})").unwrap();
    db.execute("MATCH (a:N), (b:N) WHERE a.id = 1 AND b.id = 1 CREATE (a)-[:E]->(b)")
        .unwrap();

    // Only yield out_degree
    let result = db
        .query("CALL degree_centrality('E') YIELD out_degree")
        .unwrap();
...
fn degree_centrality_bidirectional() {
    ...
    db.execute("MATCH (a:N), (b:N) WHERE a.id = 1 AND b.id = 2 CREATE (a)-[:E]->(b)")
        .unwrap();
    db.execute("MATCH (a:N), (b:N) WHERE a.id = 2 AND b.id = 1 CREATE (a)-[:E]->(b)")
        .unwrap();
